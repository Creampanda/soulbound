...
contract SoulboundToken {
    ...
    event Mint(address _soulAddress);
    event Burn(address _soulAddress);
    event Update(address _soulAddress);
    event SetProfile(address _profiler, address _soulAddress);
    event RemoveProfile(address _profiler, address _soulAddress);
    ...
    function mint(address _soulAddress, Soul memory _soulData) external {
        require(keccak256(bytes(souls[_soulAddress].identity)) == zeroHash, "Soul already exists");
        // require(msg.sender == operator, "Only operator can mint new souls");
        souls[_soulAddress] = _soulData;
        emit Mint(_soulAddress);
    }

    function burn(address _soulAddress) external {
        require(msg.sender == _soulAddress || msg.sender == operator, "Only users and issuers have rights to delete their data");
        delete souls[_soulAddress];
        for (uint i=0; i<profiles[_soulAddress].length; i++) {
            address profiler = profiles[_soulAddress][i];
            delete soulProfiles[profiler][_soulAddress];
        }
        emit Burn(_soulAddress);
    }
